...
run(function()
    ...
    AutoUpgradeStats = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
...
run(function()
    ...
    FourEXploit = GuiLibrary.ObjectsThatCanBeSaved.ExploitWindow.Api.CreateOptionsButton({
        Name = "ScytheDisabler",
        Function = function(callback)
            if callback then 
                FourEXploit.Enabled = true
                task.spawn(function()
                    local function proMoment()
                        repeat
                            local args = {
                                [1] = {
                                    ["direction"] = Vector3.new(2e9, 3e9, 4e9)
                                }
                            }
                            ReplicatedStorage.rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.ScytheDash:FireServer(unpack(args))
                            task.wait(0.1)
                        until not FourEXploit.Enabled
                    end

                    Player.CharacterAdded:Connect(proMoment)
                    proMoment()
                end)
            else
...
run(function()
    ...
    local function findNearestBed()
        ...
        for _, v in pairs(game.Workspace:GetDescendants()) do
            if v:IsA("BasePart") and v.Name:lower() == "bed" and v:FindFirstChild("Blanket") then
                local bedTeamColor = v.Blanket.BrickColor
                if bedTeamColor ~= lplr.Team.TeamColor then
    ...
    BedTP = GuiLibrary.ObjectsThatCanBeSaved.ExploitWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                ...
                local nearestBed = findNearestBed()
                if nearestBed and nearestBed:FindFirstChild("Blanket") and nearestBed.Blanket.BrickColor ~= lplr.Team.TeamColor then
                    tweenToNearestBed()
                end
            end
        end
...
run(function()
    local promomeng = {Enabled = false}
    promomeng = GuiLibrary.ObjectsThatCanBeSaved.ExploitWindow.Api.CreateOptionsButton({
        Name = "StaffDetector",
        Function = function(callback)
            if callback then
                for _, plr in pairs(game.Players:GetPlayers()) do
                    if plr:IsInGroup(5774246) and plr:GetRankInGroup(5774246) >= 2 then
                        warningNotification("Galaxyguard", "Staff Detected : " .. (plr.DisplayName and plr.DisplayName .. " (" .. plr.Name .. ")" or plr.Name), 60)
                    end
                end
            end
        end
